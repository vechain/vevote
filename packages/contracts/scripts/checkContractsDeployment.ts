...
async function checkContractsDeployment() {
  try {
    ...
    const code =
      config.vevoteContractAddress === "" ? "0x" : await ethers.provider.getCode(config.vevoteContractAddress);
    if (code === "0x") {
...
async function overrideLocalConfigWithNewContracts(
  ...
  const newConfig: AppConfig = {
    ...
    stargateNFTContractAddress: await contracts.stargateMock.getAddress(),
  };
