...
async function main() {
  ...
  console.log(
    `Verifying ${contractName} at ${contractAddress} on ${networkName} (chainId: ${chainId})...`
  );
  ...
  try {
    ...
    if (checkResult.isVerified && checkResult.data) {
      ...
      console.log(
        `   API v2 lookup: https://sourcify.dev/server/v2/contract/${chainId}/${contractAddress}?fields=all`
      );
      return;
    }
    ...
    if (verificationResult.success) {
      ...
      console.log(
        `🌐 View verified contract: https://repo.sourcify.dev/${chainId}/${contractAddress}`
      );
  ...
  for (const file of buildInfoFiles) {
    ...
    if (buildInfo.output && buildInfo.output.contracts) {
      for (const tryPath of possiblePaths) {
        if (
          buildInfo.output.contracts[tryPath] &&
          buildInfo.output.contracts[tryPath][contractName]
        ) {
...
async function pollVerificationJob(
  ...
  while (Date.now() - startTime < maxWaitTime) {
    ...
    try {
      ...
      const data = response.data;
      console.log(
        `   📊 Job status (attempt ${attempts}): ${data.isJobCompleted ? "completed" : "pending"}`
      );
  ...
  throw new Error(
    `Verification job timed out after ${maxWaitTime / 1000}s. Call this API endpoint to manually fetch the status https://sourcify.dev/server/v2/verify/${verificationId}`
  );
}
...
async function verifyContractV2(
  ...
  try {
    ...
    const job = await submitVerificationJobV2(
      chainId,
      contractAddress,
      metadata,
      copiedFiles,
