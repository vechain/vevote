...
if (args.length < 1) {
  console.error(
    "Usage from root: yarn contracts:verify:{network} <contract-address> [contract-name] [--partial-match]"
  );
  console.error("Example: yarn contracts:verify:mainnet 0x123... StargateDelegation");
  console.error("");
  console.error("Options:");
  console.error(
    "  --partial-match  Use 'match' verification instead of 'exact_match' (Sourcify v2 terminology)"
  );
  console.error("");
