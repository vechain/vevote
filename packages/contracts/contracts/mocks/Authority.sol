...
contract Authority {
  ...
  function native_add(address nodeMaster, address endorsor, bytes32 identity) public returns (bool) {
    ...
    validators[nodeMaster] = ValidatorInfo({ listed: true, endorsor: endorsor, identity: identity, active: true });
  ...
  function native_get(
    address nodeMaster
  ) public view returns (bool listed, address endorsor, bytes32 identity, bool active) {
  ...
  function revoke(address nodeMaster) public {
    require(msg.sender == executor() || !native_isEndorsed(nodeMaster), "builtin: requires executor or not endorsed");
    require(native_revoke(nodeMaster), "builtin: not listed");
    emit Candidate(nodeMaster, "revoked");
  }

  function get(address nodeMaster) public view returns (bool listed, address endorsor, bytes32 identity, bool active) {
