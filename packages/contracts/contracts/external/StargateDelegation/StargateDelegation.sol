  ...
  IStargateDelegation
{
  ...
  function _getStargateDelegationStorage() private pure returns (StargateDelegationStorage storage $) {
    assembly {
  ...
  function _claimRewards(uint256 _tokenId) internal {
    ...
    if ($.vthoToken.balanceOf(address(this)) < amountToClaim) {
      revert InsufficientVthoBalanceForRewardsClaim($.vthoToken.balanceOf(address(this)), amountToClaim);
    }

    // Update start block for rewards accumulation to the last completed delegation period
    $.rewardsAccumulationStartBlock[_tokenId] = _calculateLastCompletedPeriodEndBlock($, _tokenId);
  ...
  function setVthoRewardPerBlockForAllLevels(
    ...
    for (uint256 i; i < _vthoRewardPerBlock.length; i++) {
      if (_vthoRewardPerBlock[i].rewardPerBlock == 0) {
        revert InvalidVthoRewardPerBlock(_vthoRewardPerBlock[i].levelId, _vthoRewardPerBlock[i].rewardPerBlock);
      } else {
        // Update the vtho reward per block per each NFT level
        _getStargateDelegationStorage().vthoRewardPerBlock[_vthoRewardPerBlock[i].levelId] = _vthoRewardPerBlock[i]
          .rewardPerBlock;
      }
    }
  }
  ...
  function getVthoRewardsPerBlock() external view returns (VthoRewardPerBlock[] memory vthoRewardPerBlock) {
    StargateDelegationStorage storage $ = _getStargateDelegationStorage();
  ...
  function currentDelegationPeriodEndBlock(uint256 _tokenId) public view returns (uint256) {
    ...
    return $.rewardsAccumulationStartBlock[_tokenId] + ((completedDelegationPeriods + 1) * $.delegationPeriod);
  }
  ...
  function calculateLastCompletedPeriodEndBlock(uint256 _tokenId) external view returns (uint256 endBlock) {
  ...
  function _calculateLastCompletedPeriodEndBlock(
    ...
    if (!isDelegationActive(_tokenId)) {
      ...
      endBlock = $.rewardsAccumulationStartBlock[_tokenId] + (completedDelegationPeriods * $.delegationPeriod);
    }
