...
contract ThunderFactory is XAccessControl {
  ...
  constructor() {
    ...
    // The params of new node levels
    strengthParams[8] = TokenParameters(200000 ether, 0, 0, 0); // Flash
    strengthParams[9] = TokenParameters(50000 ether, 0, 0, 0); // Lightning
    strengthParams[10] = TokenParameters(10000 ether, 0, 0, 0); // Dawn

    // The params of validator node levels
    strengthParams[11] = TokenParameters(25000000 ether, 0, 0, 0); // Validator
  }
  ...
  function applyUpgrade(strengthLevel _toLvl) external whenNotPaused {
    ...
    require(
      uint8(token.level) + 1 == uint8(_toLvl) && _toLvl != strengthLevel.VeThorX && _toLvl <= strengthLevel.MjolnirX,
      "invalid _toLvl"
    );
  ...
  function getTokenParams(strengthLevel _level) public view returns (uint256, uint64, uint64, uint64) {
