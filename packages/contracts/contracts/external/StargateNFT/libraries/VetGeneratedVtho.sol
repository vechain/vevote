...
library VetGeneratedVtho {
  ...
  function claimRewards(DataTypes.StargateNFTStorage storage $, uint256 _tokenId) external returns (bool) {
  ...
  /// @param _tokenId The ID of the token to claim rewards for
  /// @return success True if the rewards were claimed successfully, will revert otherwise
  function _claimRewards(DataTypes.StargateNFTStorage storage $, uint256 _tokenId) internal returns (bool) {
    ...
    bool success = IERC20($.vthoToken).transfer(owner, rewardsToClaim);
    if (!success) {
  ...
  function claimableRewards(DataTypes.StargateNFTStorage storage $, uint256 _tokenId) external view returns (uint256) {
  ...
  /// @param _tokenId The ID of the token to calculate rewards for
  /// @return amount The amount of VTHO rewards available
  function _claimableRewards(DataTypes.StargateNFTStorage storage $, uint256 _tokenId) internal view returns (uint256) {
