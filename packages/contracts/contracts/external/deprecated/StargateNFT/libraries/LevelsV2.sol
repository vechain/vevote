...
import { ClockV2 as Clock } from "./ClockV2.sol";
import { DataTypesV2 as DataTypes } from "./DataTypesV2.sol";
import { ErrorsV2 as Errors } from "./ErrorsV2.sol";
...
library LevelsV2 {
  ...
  function addLevel(DataTypes.StargateNFTStorage storage $, DataTypes.LevelAndSupply memory _levelAndSupply) external {
    ...
    _checkpointLevelCirculatingSupply($, _levelAndSupply.level.id, _levelAndSupply.circulatingSupply);
  ...
  /// @dev Only the LEVEL_OPERATOR_ROLE can call this function
  /// Emits a {IStargateNFT.LevelCapUpdated} event
  function updateLevelCap(DataTypes.StargateNFTStorage storage $, uint8 _levelId, uint32 _cap) external {
    _updateLevelCap($, _levelId, _cap);
  }

  // ------------------ Getters ------------------ //

  function getLevelIds(DataTypes.StargateNFTStorage storage $) external view returns (uint8[] memory) {
    return _getLevelIds($);
  }
  ...
  function getLevels(DataTypes.StargateNFTStorage storage $) external view returns (DataTypes.Level[] memory) {
  ...
  function _updateLevelCap(DataTypes.StargateNFTStorage storage $, uint8 _levelId, uint32 _cap) internal {
    // Validate level exists
    if (!_levelExists($, $.levels[_levelId].id)) {
      revert Errors.LevelNotFound(_levelId);
    }
  ...
  /// @dev See {getLevelIds}
  function _getLevelIds(DataTypes.StargateNFTStorage storage $) internal view returns (uint8[] memory) {
  ...
  function _incrementLevelCirculatingSupply(DataTypes.StargateNFTStorage storage $, uint8 _levelId) internal {
  ...
  function _decrementLevelCirculatingSupply(DataTypes.StargateNFTStorage storage $, uint8 _levelId) internal {
    uint208 circulatingSupply = _getCirculatingSupply($, _levelId);
    _checkpointLevelCirculatingSupply($, _levelId, circulatingSupply - 1);
  }
