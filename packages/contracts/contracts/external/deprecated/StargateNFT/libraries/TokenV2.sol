...
import { LevelsV2 as Levels } from "./LevelsV2.sol";
...
library TokenV2 {
  ...
  function normalTokensCount(DataTypes.StargateNFTStorage storage $) external view returns (uint208) {
    uint208 count;
  ...
  function ownerTotalVetStaked(DataTypes.StargateNFTStorage storage $, address _owner) external view returns (uint256) {
    uint256[] memory tokenIds = _idsOwnedBy(_owner);
  ...
  function ownsXToken(DataTypes.StargateNFTStorage storage $, address _target) external view returns (bool) {
  ...
  function ownsNormalToken(DataTypes.StargateNFTStorage storage $, address _target) external view returns (bool) {
  ...
  function isXToken(DataTypes.StargateNFTStorage storage $, uint256 _tokenId) public view returns (bool) {
  ...
  function isNormalToken(DataTypes.StargateNFTStorage storage $, uint256 _tokenId) external view returns (bool) {
