...
interface IStargateNFTV2 is IERC721, IERC721Enumerable {
    // ------------------ Events ------------------ //
    ...
    function getLevel(uint8 _levelId) external view returns (DataTypesV2.Level memory);

    function getLevels() external view returns (DataTypesV2.Level[] memory);
    ...
    function getToken(uint256 _tokenId) external view returns (DataTypesV2.Token memory);

    function tokensOwnedBy(address _owner) external view returns (DataTypesV2.Token[] memory);
